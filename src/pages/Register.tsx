...
export function Register() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Não foi possível cadastrar o negócio.');
    } finally {
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl shadow-soft border border-moss/5 overflow-hidden">
            ...
            <div className="bg-moss-50/50 border-b border-moss/10 p-4 md:p-6">
              ...
              <div
                ...
                className="flex flex-col items-center gap-2 bg-moss-50/50">
                    ...
                    <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors duration-300 ${step >= s.id ? 'bg-terracotta text-white shadow-md' : 'bg-white text-moss-400 border-2 border-moss-200'}`}>
                  
                      {step > s.id ? <CheckCircle2 size={20} /> : s.icon}
                    </div>
                    <span
                  className={`text-xs font-medium hidden md:block ${step >= s.id ? 'text-moss-900' : 'text-moss-400'}`}>
            ...
            <div className="bg-moss-50/50 border-t border-moss/10 p-4 md:p-6 flex justify-between items-center">
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl font-medium transition-colors ${step === 1 ? 'text-moss-300 cursor-not-allowed' : 'text-moss-700 hover:bg-moss-100'}`}>
              
                <ChevronLeft size={20} /> Voltar
              </button>

              {submitError && <p className="text-sm text-red-600 mr-4">{submitError}</p>}
              {step < 5 ?
            <button
              onClick={nextStep}
            ...
            <button
              ...
              className="flex items-center gap-2 bg-terracotta hover:bg-terracotta-600 text-white px-8 py-3 rounded-xl font-medium transition-colors shadow-sm disabled:opacity-70">
                  ...
                  {isSubmitting ? 'Publicando...' : 'Publicar negócio'}{' '}
